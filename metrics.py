...
def capital_metrics(df: pd.DataFrame, owner_name: str = "Yo") -> dict:
    ...
    return dict(contributed=float(aportes), recovered=float(recuperos), exposed=float(exposed), recoverable_exposed=float(recoverable_exposed), sunk=float(sunk), partner_net=float(partner_net))

def simulate(rent, fixed_costs, avg_ticket, jobs, productive_days, material_pct, taxes_pct, ads, helper_cost, other_costs, owner_share_pct):
    jobs = max(int(jobs), 0); productive_days = max(int(productive_days), 1)
    ...
    return {"revenue": revenue,"variable_costs": variable_costs,"fixed_costs_total": fixed,"net_profit": net,"net_margin": net/revenue if revenue else 0,"owner_profit": owner_profit,"partner_profit": partner_profit,"owner_share_pct": owner_share_pct,"partner_share_pct": partner_share_pct,"break_even_revenue": break_even,"break_even_jobs": jobs_break_even,"daily_revenue": revenue/productive_days,"daily_net": net/productive_days}
...
def goal_math(owner_target, partner_target, owner_share_pct, net_margin_pct, productive_days):
    ...
    return {"net_needed": net_needed,"revenue_needed": revenue_needed,"daily_revenue": revenue_needed/days,"daily_net": net_needed/days}
...
def valuation(row):
    ...
    return {"assets": assets,"earnings_value": earnings_value,"intangible": intangible,"equity": equity,"owner_value": owner_value}
